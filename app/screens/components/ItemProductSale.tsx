...
export const ItemProductSale = ({item, choseItem}: IPropItemProductSale) => {
  return (
    <Pressable
      style={[styleItemProduct.container, {borderBottomWidth: 1, borderBottomColor: '#ccc'}]}
      ...
      <View style={{gap: 8}}>
        <Text style={{fontWeight: 500}}>{new Intl.NumberFormat('vi-VN').format(item.giaBan)}</Text>
      </View>
    </Pressable>
  );
};
